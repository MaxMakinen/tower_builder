...
var _dragged_ID: DragID = DragID.EMPTY
...
func set_dragged_item(item: InventorySlot, id: int) -> void:
	_dragged_item = item
	_dragged_ID = id as DragID
...
func return_backup() -> InventorySlot:
	var temp = _dragged_item
	if !previous_inventory.attempt_insert_at(_dragged_item.get_item(), previous_index, _dragged_item.get_amount()):
		print("Returning backup from dragged item failed")
		return null
...
func change_amount(amount: int) -> void:
	...
	if _dragged_item.get_amount() < 1:
		set_dragged_item(null, DragID.EMPTY)
